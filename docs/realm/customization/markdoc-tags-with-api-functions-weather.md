...
- You should already understand how to build a Markdoc tag.
If not, start with [Build a Markdoc tag](./build-markdoc-tags.md).
- You should be familiar with API functions basics.
See [Create API functions](./api-functions/create-api-functions.md).
...
This defines an endpoint at `/api/weather` that accepts a required query parameter `q` (the location).
...
Create a component that fetches from your API function and renders the result.
...
Then export it from `@theme/markdoc/components.tsx`:
