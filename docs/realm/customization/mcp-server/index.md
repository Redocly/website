...
The public endpoint is only served when `/mcp` requires authentication.
...
- A browser visit to `/mcp-public` shows the same setup page as `/mcp`, with connection snippets that point at the public endpoint and a note that it serves public content only.
