...
Never commit the `OAUTH_CLIENT_SECRET` environment variable to your repository.
...
You can offer users a way to supply a credential inside security scheme cards in Replay's **Security** tab.
Eject the `ReplaySecuritySchemeActions` component and configure it.

For example, add a sign-in button for your identity provider.
Alternatively, you can add a link to the page that issues API keys, or a note naming the credential the scheme takes.

Unlike `ReplayGate`, it doesn't restrict access.
...
### Choose where the actions appear

Your component receives every card, decides where to act, and returns `null` everywhere else.
Narrow the offer by any of the following, and combine them as you need.
...
- A single API in a catalog
- `apiId`
- Offer a token for the payments API, and leave every other API alone.

---

- A single operation
- `operationId`
- Offer a token for `listPushes`, and leave the other operations alone.

---

- A single server
- `environment`
- Ask for a credential on production, and skip the mock server, which needs none.

---

- A type of scheme
- `securityType`
- Offer a sign-in for OAuth 2.0 cards, and a key generator for API key cards.

---

- One named scheme
- `schemeId`
- Act on `PartnerKey` only, where an API documents several API key schemes.
...
- What the card asks for, named after the fields in your API description.
...
- What the scheme's credential field resolves to, or an empty string.
  An environment input counts only when the scheme's field references it.
...
- Fills the scheme's credential.
  ...
  See [How environment values work](#how-environment-values-work) for where the value lands.
...
This example reuses the `@api/login.get.ts` callback page from the `ReplayGate` example above, which posts the token back over `BroadcastChannel`.
...
// Environment names come from the servers in your API description.
const LIVE_SERVER = 'Production server';
...
// Remember the token this component issued, so a token a user pastes is left alone.
const ISSUED_TOKEN_KEY = 'replay-issued-token';
...
export function ReplaySecuritySchemeActions({
  ...
  const handleSignIn = () => {
    ...
    channel.onmessage = (event: MessageEvent) => {
      if (event.data?.type !== 'REPLAY_AUTH_DONE' || event.data.state !== state) return;
      channel.close();
      sessionStorage.setItem(ISSUED_TOKEN_KEY, event.data.access_token);
  ...
  if (apiId !== PAYMENTS_API_ID || environment !== LIVE_SERVER) {
    return null;
  }

  if (!TOKEN_TYPES.has(securityType)) {
    return null;
  }

  return value && value === sessionStorage.getItem(ISSUED_TOKEN_KEY) ? (
    <button
      onClick={() => {
        sessionStorage.removeItem(ISSUED_TOKEN_KEY);
        setValue('');
      }}
    >
      Sign out
...
Session storage outlives the card, which Replay unmounts when a user switches operation, and it carries the token across a page reload.
It also ends with the browser tab.
...
Narrowing by `operationId` or by `schemeId` takes the same shape: one more check before the component returns anything.
