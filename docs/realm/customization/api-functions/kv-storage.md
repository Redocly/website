...
See [Types](#types) for details on [KvListSelector](#kvlistselector), [KvListOptions](#kvlistoptions), and [KvListResponse](#kvlistresponse).
...
export default async function (request: Request, context: ApiFunctionsContext) {
  ...
  const { limit, cursor } = context.query;
  
  const result = await kv.list(
    { prefix: ['users'] },
    { limit: parseInt(limit, 10), cursor }
  );
...
export default async function (request: Request, context: ApiFunctionsContext) {
  ...
  const { from, to, amount } = context.query;
  ...
  try {
    const result = await kv.transaction(async (tx) => {
      ...
      if (fromAccount.value.balance < amountNumber) {
        throw new Error('Insufficient balance');
      }
      
      // Update both accounts atomically
      await tx.set(['accounts', from], {
        balance: fromAccount.value.balance - amountNumber
      });
      await tx.set(['accounts', to], {
        balance: toAccount.value.balance + amountNumber
      });
...
- KvKey
- [KvKeyPart]
- An array of key parts forming a complete key. Example: `['users', 'john']` or `['products', 42, true]`
