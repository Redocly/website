...
```bash
curl -X POST wys-webview-id.wysiwyg.cloud.redocly.com/api/notes \
...
```bash
curl wys-webview-id.wysiwyg.cloud.redocly.com/api/notes/abc-123 \
  -H "Cookie: <your-webview-cookie>"
```
...
export default async function (request: Request, context: ApiFunctionsContext) {
  ...
  // Store permanently
  const entry = await kv.set(['notes', id], note);
...
```bash
curl "wys-webview-id.wysiwyg.cloud.redocly.com/api/cache/weather?city=london" \
  -H "Cookie: <your-webview-cookie>"
```
...
```bash
curl wys-webview-id.wysiwyg.cloud.redocly.com/api/notes \
  -H "Cookie: <your-webview-cookie>"
```
...
```bash
# First page
curl "wys-webview-id.wysiwyg.cloud.redocly.com/api/notes?limit=10" \
  -H "Cookie: <your-webview-cookie>"

# Next page (use cursor from previous response)
curl "wys-webview-id.wysiwyg.cloud.redocly.com/api/notes?limit=10&cursor=eyJrIjoiWydu..." \
  -H "Cookie: <your-webview-cookie>"
```
...
```bash
curl "wys-webview-id.wysiwyg.cloud.redocly.com/api/notes?order=desc" \
  -H "Cookie: <your-webview-cookie>"
```
...
```ts
// Query across the full keyspace using explicit bounds
const result = await kv.list({
  start: ['notes', 'note-002'],
  end: ['notes', 'note-004']
});
...
curl wys-webview-id.wysiwyg.cloud.redocly.com/api/organizations/acme/members \
  -H "Cookie: <your-webview-cookie>"

# Only engineering department
curl "wys-webview-id.wysiwyg.cloud.redocly.com/api/organizations/acme/members?department=engineering" \
  -H "Cookie: <your-webview-cookie>"
```
...
curl "wys-webview-id.wysiwyg.cloud.redocly.com/api/logs?from=2024-01-15T00:00:00Z&to=2024-01-16T00:00:00Z" \
  -H "Cookie: <your-webview-cookie>"
```
...
```bash
curl -X POST wys-webview-id.wysiwyg.cloud.redocly.com/api/notes/batch \
  -H "Content-Type: application/json" \
  -H "Cookie: <your-webview-cookie>" \
...
```bash
curl -X DELETE wys-webview-id.wysiwyg.cloud.redocly.com/api/notes/abc-123 \
  -H "Cookie: <your-webview-cookie>"
```
...
```bash
curl -X POST wys-webview-id.wysiwyg.cloud.redocly.com/api/transfer \
  -H "Content-Type: application/json" \
  -H "Cookie: <your-webview-cookie>" \
...
```bash
curl -X POST wys-webview-id.wysiwyg.cloud.redocly.com/api/users \
  -H "Content-Type: application/json" \
  -H "Cookie: <your-webview-cookie>" \
...
```bash
curl wys-webview-id.wysiwyg.cloud.redocly.com/api/users/by-email/john@example.com \
  -H "Cookie: <your-webview-cookie>"
```
...
```bash
curl -X POST wys-webview-id.wysiwyg.cloud.redocly.com/api/articles/my-article/views \
  -H "Cookie: <your-webview-cookie>"
```
...
- total
- number
- Total count of entries matching the selector after applying the current cursor.
...
- get
- `<T>(key: KvKey) => Promise<T | null>`
- Retrieves a single entry by key.

---

- getMany
- `<T>(keys: KvKey[]) => Promise<(KvListEntry<T> | null)[]>`
- Retrieves multiple entries by keys.

---

- set
- `<T>(key: KvKey, value: T, options?: KvSetOptions) => Promise<KvListEntry<T> | null>`
- Creates or updates an entry.
...
curl -X POST wys-webview-id.wysiwyg.cloud.redocly.com/api/data \
  -H "Content-Type: application/json" \
  -H "Cookie: <your-webview-cookie>" \
  -d '{"key": "value"}'
```
