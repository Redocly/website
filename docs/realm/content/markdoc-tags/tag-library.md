...
Use the Visibility tag to route content to one audience: people reading the page, or AI agents reading the page's Markdown version.
Content marked `for="humans"` renders on the page but is left out of the Markdown version; content marked `for="agents"` is left out of the rendered page but included in the Markdown version.
