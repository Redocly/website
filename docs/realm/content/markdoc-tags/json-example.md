...
- A literal JSON value or a `$ref` to an existing example.
...
- schema
- JSON schema object
- A JSON schema definition or a `$ref` to an existing schema.
...
- mode
- string
- Controls property filtering when using `schema`.
...
{% markdoc-example renderDemo=true %}
  ...
  {% json-example
    value={
      "$ref": "../../openapi-files/redocly-museum.yaml#/components/examples/CreateSpecialEventRequest/value"
    }
  /%}
  ```
{% /markdoc-example %}
...
{% markdoc-example renderDemo=true %}
  ...
  {% json-example
    schema={
      "$ref": "../../openapi-files/redocly-museum.yaml#/components/schemas/SpecialEvent"
    }
  /%}
  ```
{% /markdoc-example %}
