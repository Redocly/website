...
export default function ThirdPartyLicenses() {
  ...
  const licenseCounts = React.useMemo(() => {
    const counts = new Map<string, number>(selectedLicenses.map((license) => [license, 0]));
    for (const entry of searchedPackages) {
      counts.set(entry.license, (counts.get(entry.license) ?? 0) + 1);
    }
    return [...counts.entries()].sort(([a], [b]) => a.localeCompare(b));
  }, [searchedPackages, selectedLicenses]);

  const filteredPackages = selectedLicenses.length
    ? searchedPackages.filter((entry) => selectedLicenses.includes(entry.license))
    : searchedPackages;
  ...
  const filterLabel =
    selectedLicenses.length === 0
      ...
      : selectedLicenses.length === 1
        ? selectedLicenses[0]
        : `${selectedLicenses.length} licenses`;
        ...
        <Markdown>
          ...
          <Controls>
            ...
            <Dropdown
              ...
              withArrow
              trigger={
                <Button variant="outlined" data-active={selectedLicenses.length > 0}>
                  {filterLabel}
                </Button>
              }
            >
              <DropdownMenu>
                {licenseCounts.map(([license, count]) => (
                  <DropdownMenuItem
                    key={license}
                    content={license}
                    prefix={<CheckboxIcon checked={selectedLicenses.includes(license)} />}
            ...
            {(selectedLicenses.length > 0 || searchTerm) && (
              <Button
                variant="text"
                onClick={() => {
                  setSearchTerm('');
...
const Wrapper = styled.div`
  ...
  table.${MARKDOWN_CLASS_NAME} th:first-child {
    width: 80%;
  }
`;
...
const Controls = styled.div`
  ...
  [data-component-name='Search/SearchInput'] {
    ...
    input {
      border-radius: 0;
      padding: 0 1px;
      height: var(--search-trigger-line-height);
