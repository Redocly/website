...
- projectUrl
- string
- Specifies the URL of your project in Reunite. Used for the remote scorecard feature.
  Must be a valid HTTPS URL with the format: `https://app.cloud.redocly.com/org/{orgSlug}/project/{projectSlug}`.
  When `projectUrl` is specified, the [residency](./residency.md) configuration can be omitted as the project URL already contains the residency information.
