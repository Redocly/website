...
Configure multiple catalogs to organize and display different types of entities in your documentation. Customize each catalog with rules for inclusion and exclusion, and UI filters.
...
- **REQUIRED.**
  A map of catalog configurations.
  The key for each entry is a unique catalog identifier.
...
- string
- **REQUIRED.**
  Entity property to use for filtering.
  You can take this value from `metadata` or any other top-level entity property.
...
- parentFilter
- string
- The `property` value of another filter object to use as a parent filter.
  This filter becomes active only after the parent filter is selected.
