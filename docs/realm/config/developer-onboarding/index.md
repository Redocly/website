...
For adapters with `type: APIGEE_X` or `type: APIGEE_EDGE`, the developer portal exposes **Callback URLs** in two places.
They appear when developers create an application and on the application detail page (API keys tab).
Developers can set or change redirect URLs without using the Apigee UI.
