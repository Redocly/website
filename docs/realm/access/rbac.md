...
- **Organization roles** from identity provider claims on **corporate** SSO, merged with the stored role for **guest** SSO (see [Organization role priority with SSO](./roles.md#organization-role-priority-with-sso)), or manual assignment when using Redocly login
