...
It works alongside [single sign-on (SSO)](./sso.md).
...
SCIM is in beta and may change.
...
Until then, you will not see **Enable SCIM** or **SCIM endpoint** on the identity provider form.
{% /admonition %}
...
3. Under **Identity Provider**, open the Corporate or Guest provider you want to connect to SCIM (for example, click **Edit**).
   If you use SAML 2 or OpenID Connect, you are taken to the corresponding identity provider form.
4. Select **Enable SCIM 2.0**.
5. Copy the **SCIM endpoint** value shown on the form.
...
Use the format `Bearer` followed by a space and the full API key string (the same value you copy when you create a key in **API keys**).

Create or rotate keys under **API keys** in your organization settings.
See [Manage API keys](../api-keys.md).
...
Until the IdP uses a valid key, its SCIM requests fail and automatic provisioning stops.

## Supported SCIM attributes

Redocly follows SCIM 2.0 Core User and Group resources.
Map your identity provider’s attributes to the SCIM fields below so provisioning matches what Redocly stores.
...
Exact behavior depends on which SCIM operations your IdP sends.
If something does not sync in your environment, confirm attribute mapping in your IdP matches the fields above.
...
- **[Single sign-on (SSO) concepts](./sso.md)** - How Corporate and Guest identity providers work with Reunite and projects
