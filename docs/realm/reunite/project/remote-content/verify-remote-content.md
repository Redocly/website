...
2. Changes from the remote source are applied on top of the Redocly project's default branch (`main`).
3. A preview build process runs for the `x-feature` branch to check for errors and to generate a preview of the site with the latest changes.
...
1. A branch (for example `x-feature`) is created in the Redocly project's repository.
2. Changes from the remote source are applied on top of the Redocly project's default branch (`main`).
3. A pull request from `x-feature` to the default branch is created (see [Automatically-created pull requests](#automatically-created-pull-requests)).
4. A preview build for `x-feature` branch is created to verify that changes do not break the project.
5. After successful build verification, Reunite automatically merges the open pull request (if Auto-merge is enabled) and starts a production build.
