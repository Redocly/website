...
New projects start in [draft mode](./draft-projects.md). While in draft mode, deployment URLs are protected behind a login screen, so only authenticated users can view the published project.
{% /admonition %}
