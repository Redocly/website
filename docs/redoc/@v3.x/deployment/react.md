...
Your project must already use `react` and `react-dom` 19.2.7 or later.

Install the package together with `styled-components` 6, its other peer dependency:

```bash
npm install redoc styled-components
```

`@redocly/config` is a dependency of the package and installs on its own.
...
1. Use the component, either:
    ...
    To load a description file from your own project, place it in the folder your build tool serves as static assets, for example `public` in Vite or Create React App, and point `specUrl` at its root-relative path such as `/openapi.yaml`.
    The file is fetched at runtime, so a file that is not served as a static asset returns a 404.
    To bundle the description with your application instead, import a JSON description and pass it to `spec`.
