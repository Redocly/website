...
The page works directly from disk when the script comes from the CDN and the description from a URL.
...
Scope every override to a color mode: `:root.light` for light, `:root.dark` for dark, and `:root.light, :root.dark` for values shared by both.
A plain `:root` rule has no effect, because Redoc CE adds its own `:root` styles when it starts and they override yours.

The following example sets the sidebar width for both color modes, and the accent color and the sidebar background for each one:

```css
/* Shared by both color modes */
:root.light,
:root.dark {
  --sidebar-width: 280px;
}
...
import { init } from 'https://cdn.redoc.ly/redoc/v3.x/bundles/redoc.standalone.js';
...
```js
import { hydrate } from 'https://cdn.redoc.ly/redoc/v3.x/bundles/redoc.standalone.js';
...
{% tabs %}
  {% tab label="Use CDN" %}
    ...
    The same file is available from jsDelivr, which mirrors the npm package:
      ...
      ```html
      <script type="module" src="https://cdn.jsdelivr.net/npm/redoc@3/bundles/redoc.standalone.js"></script>
      ```
  {% /tab %}
  {% tab label="Self-host dependencies" %}
    ...
    1. Reference the Redoc CE script with a node modules link.
        ...
        ```html
        <script type="module" src="node_modules/redoc/bundles/redoc.standalone.js"></script>
        ```
